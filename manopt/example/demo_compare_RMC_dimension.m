...
function info = demo_compare_RMC_dimension()
clear;
close all;
addpath ../misc
...
%             residual_Rsub = zeros(50,1);    residual_PAMAL = zeros(50,1);  residual_soc = zeros(50,1);
            ...
            for test_random = 1:20  %times average.
                ...
                A = struct();
                A.applyA = @(X) RMC_applyA(X,I,J,Xout);
                A.applyAT = @(y) RMC_applyAT(y,I,J,Xout);
                ...
                options_mialm.nu0 = svds(AtA,1)^2/2*10 ;
    ...
    function [Y] = RMC_applyA(X,I,J,Xout)
        Y = zeros(size(X));
        for i = I
            for j = J
                Y = X(i,j) - Xout(i,j);
   ...
   function [X] = RMC_applyAT(Y,I,J,Xout)
