    ...
    for id_r =2:size(r_set,1) % r  number of column
        for id_mu = 1:size(mu_set,1)         % mu  sparse parameter
            ...
            for test_random = 1:test_num  %times average.
                ...
                [phi_init1]= Re_sub_grad_spca(AtA,option_Rsub);
                ...
                options_mialm.max_iter = 200; options_mialm.tol = 1e-8*n*r;
                ...
                options_admm.max_iter = 1000;  options_admm.opt = out_mialm.obj;
               ...
               % profsave
                ...
                [X_Rsub, F_Rsub,sparsity_Rsub,time_Rsub,...
                    maxit_att_Rsub,succ_flag_sub,obj_arr_Rsub,time_arr_Rsub]= Re_sub_grad_spca(AtA,option_Rsub);
                ret_Rsub(test_random,:) = [F_Rsub,sparsity_Rsub,time_Rsub, maxit_att_Rsub, succ_flag_sub];
                
                
                 %%%%%% soc parameter
                option_soc.phi_init = phi_init; option_soc.maxiter = 20000;  option_soc.tol =1e-7*n*r;
               ...
               ret_Soc(test_random,:) = [F_soc,sparsity_soc,time_soc, maxit_att_soc, ...
                 ...
                 %%%%%% PAMAL parameter
                option_PAMAL.phi_init = phi_init; option_PAMAL.maxiter =2000;  option_PAMAL.tol =1e-7*n*r;
               ...
               ret_pamal(test_random,:) = [F_pamal,sparsity_pamal,time_pamal, maxit_att_pamal, ...
                ...
                line   = {'k-' 'b--' 'r-.' 'c:' 'g-*' 'm-' 'r-'};
                hold on
                plot(out_mialm.time_arr(1:out_mialm.iter),out_mialm.obj_arr(1:out_mialm.iter),line{1})
                plot(out_madmm.time_arr(1:out_madmm.iter),out_madmm.obj_arr(1:out_madmm.iter),line{2})
                plot(time_arr_manpg(1:maxit_att_manpg-1),obj_arr_manpg(1:maxit_att_manpg-1),line{3})
                plot(time_arr_manpg_BB(1:maxit_att_manpg_BB-1),obj_arr_manpg_BB(1:maxit_att_manpg_BB-1),line{4})
                %plot(time_arr_Rsub(1:maxit_att_Rsub),obj_arr_Rsub(1:maxit_att_Rsub),line{5})
                legend( {'MIALM','MADMM','ManPG','ManPG-adap'}, 'location','southeast');
                xlabel('time');
                ylabel('obj');
                title('The objective value compare');
                set(gca,'fontsize',16)
                 print(fig , '-depsc2', strcat('../result/spca', 'time_obj_.eps'));
                end
            end
