...
function info = demo_compare_ODL_dimension()
clear;
close all;
addpath ../misc
    ...
    for id_r = 2% :size(r_set,1) % r  number of column
        for id_mu = 2%:length(mu_set)  %mu  sparsity parameter
            ...
            for test_random =1:50  %times average.
                ...
                p1 = 600; p2  =300;
                R = mvnrnd(zeros(1,r),eye(r),n);
                C = mvnrnd(zeros(1,p1),eye(p1),r);
                O = mvnrnd(zeros(1,p2),eye(p2),n);
                [Q,~] = qr(R,0);
                X = Q*C;
                Y = [X,O];
                ...
                fprintf(fid,'----------------------------------------------------------------------------------\n');
                problem.M = stiefelfactory(p1+p2, r);
                
                
                [phi_init,~] = svd(randn(p1+p2,r),0);  % random intialization
