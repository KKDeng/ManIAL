function [X_Rsub,F_Sub,sparsity,time_Rsub,i,succ_flag,f_re_sub,time_arr] = Re_sub_grad_spca_sto(B,option)
...
for i = 2:maxiter
    ...
    %pgx = gx;
    %eta = 0.5*stepsize*1^i; 
    eta = max(1e-8,1/i^2);  
    %eta = 1/(4*i)^1;
    %eta = 0.01*0.99^i;%minist;
    %eta = 0.01*0.999^i; %random setting
    if flag==1
       eta = 2/i;
    end
