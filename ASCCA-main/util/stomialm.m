function [X,Z1,Z2,ret]=stomialm(Aop, Bop, manifold, f, h, opts)
...
while iter<opts.max_iter && ~cstop
    ...
    switch sub_solver
        ...
        case 3
           ...
           optRGB.stochastic  = 1;
           [X, ~, out] = stoRGBB(X0, @fun_ARNT, manifold, optRGB, Aop, Bop, f, h, Z1, Z2, nuk, m, batchsize);
    ...
    cstop = (kkt_error<tol  && deltak < tol ) || obj<opts.opt + 1e-8 || toc(t) > 50 || abs(obj_arr(iter+1)-obj_arr(iter))<1e-3*abs(obj_arr(iter));
    if cstop
        ret.flag = 0;
        ret.msg = 'converge';
    end
...
ret.Y = P;
ret.nu = nuk;
ret.obj = f.cost_grad(X,f.data{:},all_idx) + h.cost(Aop.applyA(X.U),Bop.applyB(X.V),h.data{:});
...
function [f,g,gpre] = fun_ARNT(X, Xpre, Aop, Bop, f, h, Z1,Z2,nuk, m, batchsize)
...
[f2,g2] = f.cost_grad(Xpre,f.data{:},idx_batch);
...
gpre.U = g2.U + nuk*AAXZ.U;  gpre.V = g2.V + nuk*AAXZ.V;
