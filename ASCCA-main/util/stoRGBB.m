function [x, f, out] = stoRGBB(x, fun, M, opts, varargin)
...
[f,ge,gepre] = feval(fun, x, x, varargin{:});
...
for iter = 1:maxit
    ...
    while 1
        ...
        x = M.retr(xp, d, alpha);
        [f,ge,gepre] = feval(fun, x, xp, varargin{:});
    ...
    if opts.stochastic == 1
        a = 1/iter^2;
    else
        a = 0;
    end
    ve.U = ge.U + a*(ve.U - gepre.U);
    ve.V = ge.V + a*(ve.V - gepre.V);
    v = M.egrad2rgrad(x,ve);
    % Riemannian gradient
    g = M.egrad2rgrad(x,ge); 
